"""
OpenDSS Engine for Self-Healing Switch Control
================================================
Provides switch control, energized load tracking, and voltage monitoring
for the self-healing component. Uses the same global opendssdirect instance
as opendss_service.py (singleton COM object).

Unlike the feature backend version, this uses dss.Basic.DataPath()
instead of os.chdir() to avoid changing the process CWD.

All bus names returned are LOWERCASE.
"""
...
from config.grid_config import (
    ALL_SWITCH_NAMES, TIE_SWITCH_NAMES,
...
class OpenDSSEngine:
    ...
    def load_model(self, master_path: Optional[str] = None) -> bool:
        path = Path(master_path) if master_path else get_dss_master_path()
        if not path.exists():
            logger.error(f"Master.dss not found at {path}")
            return False

        # Use DataPath instead of os.chdir (safe, no global CWD change)
        dss.Basic.DataPath(str(path.parent))
        ...
        self._loaded = True
        logger.info(
            f"Self-healing engine loaded: {dss.Circuit.Name()}, "
            f"{dss.Circuit.NumBuses()} buses, {dss.Circuit.NumCktElements()} elements"
        )
        return True
